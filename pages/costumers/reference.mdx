...
```
`id`: <u>string</u><br /> ID público do cliente

### user_id:
```json
"user_id": "user_xpto"
```
`user_id`: <u>string</u><br /> Identificador único do usuário dono do cliente

### name:
`name`: <u>string</u><br /> Nome do cliente

### email:
`email`: <u>string</u><br /> E-mail do cliente (obrigatório)

### cpf:
`cpf`: <u>string</u><br /> CPF do cliente

### phone:
`phone`: <u>string</u><br /> Telefone do cliente

### created_at / updated_at:
`created_at`, `updated_at`: <u>string (ISO date)</u><br /> Datas de criação e atualização do cliente
