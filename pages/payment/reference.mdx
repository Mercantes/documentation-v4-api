...
Para cobrançar `ONE_TIME` é obrigatório informar o cliente ao criar a cobrança por meio dos campos `customerId` ou `customer`. Só é necessário fornecer uma das opções de identificador do cliente: `customerId` **OU** `customer`.
